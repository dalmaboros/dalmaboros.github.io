...
In the words of the Dan Abramov, co-creator of Redux, "Local state is fine."
...
2. I still want to handle the click event with `handleOnClick`, but now I'll use one piece of the local state, `isMatch`, to set another piece of the local state, `modalVisible`. 
...
3. Where then, is `isMatch` determined now? I move the logic of the old `isMatch()` function into React's `static getDerivedStateFromProps()` lifecycle method, and this is where the `isMatch` part of state is set:
...
4. The `handleClose` method is modified to instead of calling a dispatch, to set the `modalVisible` part of state to false:
...
6. Since the `hideModal` and `showModal` action creators are no longer used, they can be deleted from `clothesActions.js`, so this:
...
7. We can thus delete the `'SHOW_MODAL'` AND `'HIDE_MODAL'` cases from `clothesReducer.js`. We can also delete `modalVisible` from `initialState`:
